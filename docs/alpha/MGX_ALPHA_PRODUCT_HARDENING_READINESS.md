...
`HOLD_FOR_CORE5_BROWSER_MATRIX_LIVE_TOURNAMENT_RESULT_PATH_REMOTE_SYNC_AND_PHYSICAL_QA`

Badugi focused raise/call no-reraise closure is P0-clean in the new browser trace, and the Browser Gameplay Invariant Badugi ladder now passes Badugi cash desktop 1-hand, 10-hand, and 100-hand runs. The earlier hand16 100-hand halt is classified as progress-helper stale-read / terminal and next-hand detection, with the focused repro now completing through hand20. The allowed Badugi-only cash/tournament x desktop/portrait/landscape matrix also completed 120/120 hands. Step6 cleared the Badugi portrait mobile UI blocker, Step7 cleared the earlier automated long-run active-pot / terminal-transition restore gate, and live action-history audit confirms no Core5 actor-order violation in that older harness. Friend alpha is still blocked because the Core5 browser matrix has not yet been expanded, live Core5 alpha smoke fails to reach tournament result/next-hand for D01/D02/S01/S02, remote sync is unresolved, and physical mobile QA is pending.
...
| Browser gameplay invariant gate | BADUGI_PASS / CORE5_PENDING | Badugi cash desktop 1/10/100-hand ladder passes; Badugi-only cash/tournament x desktop/portrait/landscape 20-hand matrix passes with P1 cash pot timing monitor |
...
| P1 | Expand Browser Gameplay gate to the next approved Core5 step | Badugi ladder is clean for P0, but Core5 browser gameplay has not yet been proven action-by-action |
