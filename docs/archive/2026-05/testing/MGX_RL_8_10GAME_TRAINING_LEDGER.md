...
| `npm run ai:plan-10game-rl -- --report docs/testing/MGX_10GAME_RL_READINESS_REPORT.md --fail-on-blocking` | PASS | GAME-ALL-03: fixed 10-Game rotation 10 variants / 20 Beginner+Standard routes OK; per-variant dataset, reward, action mask, short evaluation, and 50k long-run entries recorded |
| `PYTHONPATH=src .venv/bin/python -m pytest src/rl/__tests__/test_ten_game_rl_plan.py` | PASS | 10-Game target list, route status, assets/shapes, and Badugi Beginner generic fallback policy locked |
| `npm run test:rl:safety` | PASS | RL resume safety remains green before long-run training; 52 tests passed |
| `npm run test:game:one-hand` | PASS | One-hand progression guarantee remains green; 53 tests passed |
| `npm run build` | PASS | Existing browserslist/chunk-size warnings only |
...
- `docs/testing/MGX_10GAME_RL_READINESS_REPORT.md` の `Long RL ready=YES` は、Beginner/Standardの学習導線とroutingが揃っているという意味。Pro/Iron/WorldMaster強度や人間相手の勝率保証ではない。
