...
| `MOBILE_LAYOUT_MODE_SEPARATION` | Portrait and landscape mobile tournament layouts need different information architecture. The current good landscape state should not be diluted by portrait-responsive interpolation. | Physical iPhone landscape QA; `docs/ui/MGX_MOBILE_LAYOUT_POLICY.md`; `reports/ui/mobile-layout-policy/mobile-battlefield-ratio.json`. | P1 | Mobile layout decisions previously flowed from generic responsive classes, so portrait bottom-stack needs and landscape side-column needs were coupled. | Use explicit `mobile-portrait` and `mobile-landscape` modes. Portrait reserves table height and bottom controls; landscape reserves table width and right compact utility column. | S-M | High: prevents future HUD/control regressions in one orientation while tuning the other. |
