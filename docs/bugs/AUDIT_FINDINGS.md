...
| `EV-GUARD-07` | Strict chip conservation should be enabled for every controller. | Strict pot/payout/chip conservation is enforced for Board B01-B09 and Stud ST1-ST6 (15/36). Draw, Dramaha, and CP1 retain explicit family allowlist reasons. | Potential EV/reward contamination remains outside the 15 enforced variants. | Normalize the remaining terminal snapshots and remove allowlist entries family by family. | Non-blocking for current friend alpha; important integrity gap. |
