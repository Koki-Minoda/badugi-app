...
| `DRAW-NAT-01` | Draw UI E2E | Natural Draw#1-#3 UI path now reaches result in targeted Playwright, but manual preview/mobile validation is still pending. | P2 | `tests/e2e/badugi-flow.spec.ts`; `tests/e2e/badugi-full-round-pot-regression.spec.ts` PASS | Re-run on preview URL before changing Badugi availability. |
