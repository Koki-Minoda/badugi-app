...
Physical iPhone QA later reopened the cross-variant blocker: after draw-lowball cash play and a menu return, Badugi tournament could display five-card hands and then freeze in DRAW after Hero folded. This is a stronger contamination signature than the prior controller-class mismatch because Badugi may report the correct active variant while stale D01/D02/S01/S02 player hand shape survives in the merged snapshot. Local hardening now rejects hand-shape-incompatible snapshots, sanitizes stale player cards at variant/mode reset boundaries, and prevents folded/out/busted seats from being elected as DRAW actors. The physical-flow regression passes locally for D01/D02/S01/S02/Badugi cash -> Badugi tournament. Track `BADUGI-HAND-SHAPE-001`, `BADUGI-FOLD-DRAW-FREEZE-001`, and reopened `CROSS-VARIANT-STATE-001` as `FIXED_LOCAL / NEEDS_DEPLOY / NEEDS_PHYSICAL_RECHECK`.
