...
Physical iPhone QA later reopened the cross-variant blocker: after draw-lowball cash play and a menu return, Badugi tournament could display five-card hands and then freeze in DRAW after Hero folded. This is a stronger contamination signature than the prior controller-class mismatch because Badugi may report the correct active variant while stale D01/D02/S01/S02 player hand shape survives in the merged snapshot. Deployed hardening now rejects hand-shape-incompatible snapshots, sanitizes stale player cards at variant/mode reset boundaries, and prevents folded/out/busted seats from being elected as DRAW actors. The physical-flow regression passes live for D01/D02/S01/S02/Badugi cash -> Badugi tournament on deployed head `a73042dcd2eca92097aa27a2d3f732b648fd49f4`. Track `BADUGI-HAND-SHAPE-001`, `BADUGI-FOLD-DRAW-FREEZE-001`, and reopened `CROSS-VARIANT-STATE-001` as `FIXED_LIVE / NEEDS_PHYSICAL_RECHECK`.
