"""Export board betting DQN checkpoints to frontend ONNX assets."""
...
def checkpoint_for(family: str, tier: str) -> Path:
    return PROJECT_ROOT / f"rl/models/board_{family}_{tier}_20260505/{family}_{tier}_board_dqn_latest.pt"
...
def upsert_registry_entry(registry_path: Path, *, family: str, tier: str, output: Path, checksum: str, checkpoint: Path):
    ...
    entry = {
        ...
        "trainingRun": f"board_{family}_{tier}_dqn_20260505",
        ...
        "featureSet": "board-betting-observation-v1",
...
def parse_args():
    parser = argparse.ArgumentParser(description="Export board DQN checkpoint to ONNX.")
...
def main():
    args = parse_args()
    result = export_board_checkpoint(
        family=args.family,
        tier=args.tier,
