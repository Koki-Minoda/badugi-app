...
const REPORT_PATH = path.resolve("reports/alpha/core5-mobile-tournament-landscape-layout.json");
const LANDSCAPE_VIEWPORTS = TOURNAMENT_VIEWPORTS.filter((viewport) => viewport.orientation === "landscape");

test.describe("Core 5 mobile tournament landscape layout", () => {
  ...
  for (const variant of CORE5_VARIANTS) {
    for (const viewport of LANDSCAPE_VIEWPORTS) {
      test(`${variant.game} tournament landscape ${viewport.name} avoids seat/card/control collisions`, async ({ browser }) => {
        ...
        try {
          ...
          screenshotPath = await saveTournamentScreenshot({
            page,
            prefix: "core5-mobile-tournament-landscape",
            variant,
            viewport,
          });
        } catch (error) {
          issues.push({
            priority: "P0",
            issue: "tournament landscape layout audit threw",
