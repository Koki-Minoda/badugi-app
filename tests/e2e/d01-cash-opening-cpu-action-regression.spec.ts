...
const REPORT_DIR = path.resolve("reports/d01");
const TRACE_PATH = path.join(REPORT_DIR, "d01-cash-opening-cpu-action-trace.jsonl");
const SUMMARY_PATH = path.join(REPORT_DIR, "d01-cash-opening-cpu-action-summary.json");
...
async function openD01Cash(page: Page) {
  await page.addInitScript(() => {
    window.localStorage.setItem("mgx.previewVariants", "true");
  });
  await page.setViewportSize({ width: 390, height: 844 });
  await openAuthenticatedGame(page, `${APP_URL}?variant=D01&mode=cash&mgxQa=mobile`);
...
test("D01 cash opening CPU actor applies an action instead of freezing the hand start", async ({ page }) => {
  test.setTimeout(120000);
  ensureReportDir();
  fs.writeFileSync(TRACE_PATH, "");
  await openD01Cash(page);
  ...
  expect(initial?.actor, "D01 opening actor should be elected").not.toBeNull();
  ...
  fs.writeFileSync(
    SUMMARY_PATH,
    `${JSON.stringify(
      { generatedAt: new Date().toISOString(), initial: compact(initial), after: compact(after), failure, debugAfter },
      null,
      2,
    )}\n`,
  );
