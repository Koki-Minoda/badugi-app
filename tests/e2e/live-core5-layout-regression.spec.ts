...
const REPORT_PATH = path.resolve("reports/alpha/live-core5-layout-evidence.json");

const LAYOUT_CASES = [
  ...LIVE_LAYOUT_VIEWPORTS.filter((viewport) => viewport.orientation === "portrait").map((viewport) => ({
    mode: "cash" as const,
    viewport,
  })),
  ...LIVE_LAYOUT_VIEWPORTS.filter((viewport) => viewport.orientation === "portrait").map((viewport) => ({
  ...
  ...LIVE_LAYOUT_VIEWPORTS.filter((viewport) => viewport.orientation === "landscape").map((viewport) => ({
    mode: "tournament" as const,
    viewport,
  })),
];

test.describe("Live Core5 layout evidence", () => {
  ...
  for (const variant of CORE5_VARIANTS) {
    for (const layoutCase of LAYOUT_CASES) {
      test(`${variant.game} live ${layoutCase.mode} ${layoutCase.viewport.name}`, async ({ browser }) => {
