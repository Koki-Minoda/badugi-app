...
test.describe("2-7 Triple Draw progression spec", () => {
  test.describe.configure({ timeout: 180000 });

  test("D01 reaches hand result through exactly three draw rounds", async ({ page }) => {
    ...
    await expect(page.getByText(/2-7 Triple Draw/i).first()).toBeVisible({ timeout: 20000 });
    ...
    const variantText = [
      ...
      .filter(Boolean)
      .join(" ");
    expect(variantText).toMatch(/DeuceToSevenTripleDraw|D01|2-7 Triple Draw/i);
